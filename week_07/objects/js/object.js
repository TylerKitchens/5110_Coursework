...
for (let i = 0; i < nameArray.length; i++) {
    nameList.innerHTML += "<option id='" + nameArray[i] + "'>" + nameArray[i]  + "</option>"
}
...
function quote() {
    ...
    if (classObject.hasOwnProperty(sName)) {
        document.querySelector("#image").innerHTML = "<image src='./pix/" +sName + ".jpg' />"
        document.querySelector("#aboutme").innerHTML = classObject[sName]
    } else {
        document.querySelector("#aboutme").innerHTML = document.querySelector("#aname").value + " is not in the class"
    }
